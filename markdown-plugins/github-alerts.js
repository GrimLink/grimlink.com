...
// Path data for GitHub's octicons, matched 1:1 to remark-github-blockquote-alert
// so src/assets/gh-alert.css keeps working unchanged.
...
function alertIcon(type) {
	return {
		...
		children: [
			{
				type: "emphasis",
				data: { hName: "path", hProperties: { d: ALERT_ICON_PATHS[type] } },
				children: [],
			},
		],
	};
...
export const githubAlerts = defineMdastPlugin({
	...
	blockquote(node, ctx) {
		...
		const updatedFirstParagraphChildren = firstText.value.includes("\n")
			? [
					{ type: "text", value: firstText.value.replace(ALERT_REGEX, "").replace(/^\n+/, "") },
					...firstParagraph.children.slice(1),
				]
			: firstParagraph.children.filter(
					(child, index) => !(index === 0 || (index === 1 && child.type === "break")),
				);

		const titleParagraph = {
			type: "paragraph",
			data: { hProperties: { className: "markdown-alert-title", dir: "auto" } },
			children: [alertIcon(alertType), { type: "text", value: alertType.toUpperCase() }],
		};

		ctx.setProperty(node, "children", [
			titleParagraph,
			{ ...firstParagraph, children: updatedFirstParagraphChildren },
